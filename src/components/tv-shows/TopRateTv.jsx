...
import { fetchTopRatedTv } from "../../features/tv-actions/tvAction";
...
export default function TopRateTv() {
     ...
     let { topRatedTv, page } = useSelector((state) => state.myTvs);
     const totalPage = topRatedTv?.total_pages || 1;
          ...
          <>
               <div className="my-[120px]">
                    ...
                    <div className="flex justify-between items-center my-4 px-16">
                         <h2 className="text-base md:text-xl xl:text-2xl">Top Rated TV Shows</h2>
                         <Pagination
                              currentPage={page}
                    ...
                    <section className="grid grid-cols-6 gap-10 px-16 ">
                         {
                              topRatedTv?.results?.length > 0 ? (
                                   topRatedTv?.results?.map(item => {
