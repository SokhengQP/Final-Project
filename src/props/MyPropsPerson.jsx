...
export function MyPropsPerson({ name, profile, departments, overviews }) {
    ...
    return (
        <div className="flex gap-6 relative h-[300px] group mt-24">
            <img
                className={`hover:scale-105 absolute rounded-[50%] left-2/4 top-6 w-[150px] -translate-x-2/4 -translate-y-2/4 sm:rounded-3xl sm:w-[50%] sm:h-[60%] object-cover md:left-12 md:top-2/4 md:-translate-y-2/4 md:w-[220px]  md:rounded-3xl aspect-square  ${!isLoaded ? 'blur-xl' : ''}`}
                src={profile}
                alt={name}
